...
class RoundRobin:
    ...
    def select_server(self):
        self.count += 1
        return self.servers[self.count % len(self.servers)]
...
class LeastConnections:
    def __init__(self, servers):
        self.servers = servers #quantos ligações ativas para cada um dos servidores (lista,mapa,dicionario) incrementamos o valor no select server, no update decrementamos o valor
        self.active_conns = {}
        for server in self.servers:
            self.active_conns[server] = 0
    def select_server(self):
        leastConServer = min(self.active_conns, key=self.active_conns.get)
        self.active_conns[leastConServer] += 1
        return leastConServer

    def update(self, *arg):
        self.active_conns[arg[0]] -= 1
...
class LeastResponseTime:
    ...
    def select_server(self):
        min_avg = min(self.avgTime.values())
        print('min', min_avg)
        eligible = []
        for server in self.avgTime.keys():
            if self.avgTime[server] == min_avg:
        ...
        if len(eligible) == 1:
            ...
            min_conns = min(self.conns.values())
            eligible_conn = []
            for server in self.conns.keys():
                if self.conns[server] == min_conns:
                    eligible_conn.append(server)
            print('a',eligible_conn )
            server = eligible_conn[0]
